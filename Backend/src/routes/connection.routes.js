...
router.post("/:requestId/respond", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (status === "accepted") {
      ...
      // **** Add this match count check here ****
...
// inside router.get("/accepted", authMiddleware, ...)
router.get("/accepted", authMiddleware, async (req, res) => {
  const userId = String(req.user.id);
  try {
    // Get all accepted connection requests involving current user
    ...
    // Map and normalize so the 'otherUser' is explicit for the frontend
    const normalized = accepted.map((conn) => {
      ...
      let otherUser = null;
      if (senderId === userId) {
        otherUser = conn.receiverUserId;
      } else if (receiverId === userId) {
        otherUser = conn.senderUserId;
      }
